...
class HH_neuron(nn.Module):
    ...
    def zeros_state(self, size):
        zero_state = [torch.zeros(*size).to(device)]*6
        return zero_state

    def update_neuron(self, inputs, states=None):
        if states is None:
            v, y, m, h, mCaL, hCa = torch.zeros_like(inputs).to(device),torch.zeros_like(inputs).to(device),torch.zeros_like(inputs).to(device),torch.zeros_like(inputs).to(device),torch.zeros_like(inputs).to(device),torch.zeros_like(inputs).to(device)
        else:
            v, y, m, h, mCaL, hCa = states
            v, y, m, h, mCaL, hCa = v.to(device), y.to(device), m.to(device), h.to(device), mCaL.to(device), hCa.to(device)
        ...
        new_v = v + (inputs - I_Na - I_K - I_L - I_Ca) * self.dt  # emit C_m =1
        ...
        new_state = (new_v, new_n, new_m, new_h, new_mCaL, new_hCa)

...
class HH_neuron(nn.Module):
    def __init__(self, in_planes, out_planes):
        ...
        self.alpha_Ca = 0.055
        self.beta_Ca = 0.94
        self.alpha_hCa = 0.000457
        self.beta_hCa = 0.0065

        self.alpha_CAN = 0.0002
        self.Ca_half = 5e-4
        self.dt = 1e-1

    def zeros_state(self, size):
        zero_state = [torch.zeros(*size).to(device)] * 8  # Fixed to 8 elements
        return zero_state

    def update_neuron(self, inputs, states=None):
        if states is None:
            v, y, m, h, mCaL, hCa, Ca_i, mCAN = [torch.zeros_like(inputs).to(device) for _ in range(8)]
        else:
            v, y, m, h, mCaL, hCa, Ca_i, mCAN = states
            v, y, m, h, mCaL, hCa, Ca_i, mCAN = v.to(device), y.to(device), m.to(device), h.to(device), mCaL.to(device), hCa.to(device), Ca_i.to(device), mCAN.to(device)
        ...
        g_K = self.gbar_K * (y ** 4)
        g_Ca = self.gbar_Ca * mCaL**2 * hCa

        I_Na = g_Na * (v - self.V_Na)
        I_K = g_K * (v - self.V_K)
        I_L = self.gbar_L * (v - self.V_L)
        I_Ca = g_Ca * (v - self.V_Ca)
        I_CAN = self.gbar_CAN * mCAN**2 * (v - 20)

        new_v = v + (inputs - I_Na - I_K - I_L - I_Ca - I_CAN) * self.dt
        new_n = y + (a_n * (1 - y) - b_n * y) * self.dt
        new_m = m + (a_m * (1 - m) - b_m * m) * self.dt
        new_h = h + (a_h * (1 - h) - b_h * h) * self.dt

        new_mCaL = mCaL + (self.alpha_Ca * (1 - mCaL) - self.beta_Ca * mCaL) * self.dt
        new_hCa = hCa + (self.alpha_hCa * (1 - hCa) - self.beta_hCa * hCa) * self.dt

        mCAN_inf = self.alpha_CAN * (Ca_i / self.Ca_half)**2
        tau_CAN = 1 / (self.alpha_CAN + 0.0002)
        new_mCAN = mCAN + self.dt * ((mCAN_inf - mCAN) / tau_CAN)

        spike_out = act_fun(new_v - thresh)
        new_state = (new_v, new_n, new_m, new_h, new_mCaL, new_hCa, Ca_i, new_mCAN)
